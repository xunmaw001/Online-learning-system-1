<template>
  <div class="main-content">
		    ...
		    label="联系电话">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('xuesheng','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('xuesheng','学生积分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xueshengjifenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'学生积分':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','学生积分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xueshengjifenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'学生积分':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','学生积分') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xueshengjifenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'学生积分':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <xueshengjifen-cross-add-or-update v-if="xueshengjifenCrossAddOrUpdateFlag" :parent="this" ref="xueshengjifenCrossaddOrUpdate"></xueshengjifen-cross-add-or-update>
...
import xueshengjifenCrossAddOrUpdate from "../xueshengjifen/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                searchForm: {
      ...
      addOrUpdateFlag:false,
                  xueshengjifenCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            xueshengjifenCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xueshengjifenCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
        this.$refs.xueshengjifenCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                                        },
